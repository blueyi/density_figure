...
//                    MessageBox.Show("划线未完成，请重新划线！");
        ...
        /// <param name="start">起点</param>
        /// <param name="end">终点,起点必须在终点上方</param>
        /// <returns>返回截取后的图像路径</retu
        public static string picCutFunction(PictureBox picBox, Bitmap pic, Point start, Point end)
        {
            //-------debug-----
            //            MessageBox.Show(start.ToString() + "  end:"  + end.ToString());
            string picName = "original";
            try
            {
        ...
        /// <param name="grayValue">灰度值</param>
        /// <returns>返回灰度处理后的图像路径</returns>
        public static string picGrayScale(Bitmap pic, int grayValue)
        {
            string picName = "original";
            try
            {
        ...
        /// <param name="maxIceArea">最大连通区域面积</param>
        /// <param name="minIceArea">最小连通区域面积</param>
        public static void picCalculate(Bitmap pic, ref int iceBlockNum, ref double iceDensity, ref int maxIceArea, ref int minIceArea, ref int pixelSquare)
        {
            try
            {
                int icenum = 0;
